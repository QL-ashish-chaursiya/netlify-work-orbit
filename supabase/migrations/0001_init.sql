...
-- Migration 0001: base schema, applied from the project-root schema.sql brief.
-- FIX vs. source brief: trg_subscriptions_updated_at removed below — it
-- targeted a `subscriptions` table that does not exist in this schema
-- (billing is intentionally out of scope), so the raw brief would abort
-- mid-migration. See 0002_supplementary_rls_and_guards.sql for the missing
-- RLS policies and business-rule triggers this file's own comments flag as TODO.
-- ============================================================================
...
-- trg_subscriptions_updated_at intentionally omitted: no `subscriptions` table exists (billing out of scope).
